...
{
    // We define our own maximum and minimum functions because of CImg and Eigen doing weird stuff
    ...
    Eigen::Array2f maximum2f(Eigen::Array2f a, Eigen::Array2f b) {
        return Eigen::Array2f( maximum( a(0), b(0) ) , maximum( a(1), b(1) ) );
    }
    Eigen::Array2i maximum2i(Eigen::Array2i a, Eigen::Array2i b) {
        return Eigen::Array2i(maximum(a(0), b(0)), maximum(a(1), b(1)));
    }
    ...
    Eigen::Array2f minimum2f(Eigen::Array2f a, Eigen::Array2f b) {
        return Eigen::Array2f( minimum( a(0), b(0) ) , minimum( a(1), b(1) ) );
    }
    Eigen::Array2i minimum2i(Eigen::Array2i a, Eigen::Array2i b) {
        return Eigen::Array2i(minimum(a(0), b(0)), minimum(a(1), b(1)));
    }
    ...
    Pixel fragment_shader(const VertShaderOut& fsin, std::vector<float>& color, const cimg_library::CImg<uint8_t>& texture)
    {
        Pixel pixel;
...
#pragma omp parallel for
    ...
    void rasterize_triangle(Framebuffer& fb, const std::array<VertShaderOut, 3>& verts, std::vector<float>* color, cimg_library::CImg<uint8_t>* texture, cimg_library::CImg<uint8_t>* alphaTexture)
    {
        ...
        Eigen::Array2f bb_ul = minimum2f(minimum2f(ss1, ss2), ss3);
        Eigen::Array2f bb_lr = minimum2f(minimum2f(ss1, ss2), ss3);
        ...
        ibb_ul = maximum2i(ibb_ul, Eigen::Array2i(0, 0));
        ibb_lr = maximum2i(ibb_lr, isz);
            ...
            for (int x = ibb_ul[0]; x < ibb_lr[0]; x++)
            {
                ...
                if ((bary.array() < 1.0f).all() && (bary.array() > 0.0f).all())
                {
                    ...
                    if (po.depth() < d && d < 1.0)
                    {
                        ...
                        po = fragment_shader(v, *color, *texture);
    ...
    void rasterize(Framebuffer& fb, std::vector<int>* indices, std::vector<VertShaderOut>* verts, std::vector<float>* color, cimg_library::CImg<uint8_t>* texture, cimg_library::CImg<uint8_t>* alphaTexture)
    {
#pragma omp parallel for
        for (std::size_t i = 0; i < indices->size(); i += 3)
        {
            std::array<VertShaderOut, 3> tri{ { verts->at(indices->at(i)), verts->at(indices->at(i + 1)), verts->at(indices->at(i + 2)) } };
            rasterize_triangle(fb, tri, color, texture, alphaTexture);
        }
    }
    ...
    bool write_framebuffer(const Framebuffer& fb, const std::string& filename)
    {
        ...
        for (size_t i = 0; i<fb.width*fb.height; i++)
        {
            for (int c = 0; c<3; c++)
            {
                pixels[3 * i + c] = maximum(0.0f, minimum(fbdata[4 * i + c] * 255.0f, 255.0f));
            }
        }
